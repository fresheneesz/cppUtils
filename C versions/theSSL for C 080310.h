/* Windows Must link to static libraries:
-lssleay32
-leay32	
*/

/* Linux Must link to static libraries:
-lssl
*/
...
{	RSA* key;
	...
	void RSAkeyConstruct(RSAkey* a)
	{	a->key = RSA_new();
	}
	
	void RSAkeyDestruct(RSAkey* a)
	{	if(a->key != 0)
		{	RSA_free(a->key);
		}
	}
	...
	// encrypted messages will be 'bytes' long
	void generate(RSAkey* a, int bytes)
	{	BIGNUM* e = BN_new();
		...
		*/
		RSA_generate_key_ex(a->key, bytes*8, e, 0);
	}
	
	// generate a key of size 'bytes' with arbitrary e (MUST be an odd number)
	// encrypted messages will be 'bytes' long
	bool generateWexp(RSAkey* a, int bytes, int exponent)
	{	if(exponent%2 == 0)
	...
	bool generateWhexp(RSAkey* a, int bytes, char* hexponent)
	{	BIGNUM* e = BN_new();
	...
	int RSAsize(RSAkey* key)
	{	return RSA_size(key->key);
	}
	
	// loads PEM formated private (and public) keys
	bool loadPEMpriv(RSAkey* a, char* data, int len)
	{	BIO *keymem;
	...
	bool loadPEMpub(RSAkey* a, char* data, int len)
	{	BIO *keymem;
        keymem = BIO_new_mem_buf(data, len);
	...
	bool loadPEMpubAlt(RSAkey* a, char* data, int len)
	{	BIO *keymem;
        keymem = BIO_new_mem_buf(data, len);
		if(keymem == 0)
			return true;
	...
	bool savePEMpub(RSAkey* a, char* filename)
	{	FILE* keyfile = fopen(filename, "w");
		if(keyfile==0)
			return true;
		if(PEM_write_RSAPublicKey(keyfile,a->key) == 0)
			return true;	// fail
		fclose(keyfile);
		return false;
	}
	
	bool savePEMpriv(RSAkey* a, char* filename)
	{	FILE* keyfile = fopen(filename, "w");
		if(keyfile==0)
			return true;
		if(PEM_write_RSAPrivateKey(keyfile,a->key,0,0,0,0,0) == 0)
			return true;	// fail
		fclose(keyfile);
		return false;
	}
	...
	int encPub(RSAkey* a, char* message, int mlen, char* encryptedMessage)
	{	// the buffer 'back' must be at least the size of the return value of RSA_size(keys)
		...
		return RSA_public_encrypt(mlen, (unsigned char*)message, (unsigned char*) encryptedMessage, a->key, RSA_PKCS1_PADDING/*RSA_PKCS1_OAEP_PADDING*/);
	}
	
	// returns size of encrypted message
	// return -1 on fail - uses RSA_PKCS1_OAEP_PADDING which might be slower than others
	int encPriv(RSAkey* a, char* message, int mlen, char* encryptedMessage)
	{	// the buffer 'back' must be at least the size of the return value of RSA_size(keys)
		// flen (first argument must be LESS than RSA_size(rsa) - 11 for the PKCS #1 v1.5 based padding modes, 
		// less than RSA_size(rsa) - 41 for RSA_PKCS1_OAEP_PADDING 
		// and exactly RSA_size(rsa) for RSA_NO_PADDING.
		return RSA_private_encrypt(mlen, (unsigned char*)message, (unsigned char*) encryptedMessage, a->key, RSA_PKCS1_PADDING/*RSA_PKCS1_OAEP_PADDING*/);
	}
	
	// return -1 on fail - uses RSA_PKCS1_OAEP_PADDING which might be slower than others
	bool decPub(RSAkey* a, char* encryptedMessage, int encryptedmlen, char* originalMessage)
	{	return RSA_public_decrypt(encryptedmlen, (unsigned char*)encryptedMessage, (unsigned char*)originalMessage, a->key, RSA_PKCS1_PADDING/*RSA_PKCS1_OAEP_PADDING*/);
	}
	
	// return -1 on fail - uses RSA_PKCS1_OAEP_PADDING which might be slower than others
	bool decPriv(RSAkey* a, char* encryptedMessage, int encryptedmlen, char* originalMessage)
	{	return RSA_private_decrypt(encryptedmlen, (unsigned char*)encryptedMessage, (unsigned char*)originalMessage, a->key, RSA_PKCS1_PADDING/*RSA_PKCS1_OAEP_PADDING*/);
	}
	
	// put public exponent into string
	char* hexE(RSAkey* a, char* b)
	{	ScopyforSSL(b, BN_bn2hex(a->key->e));
		return b;
	}
	
	// put private exponent into string
	char* hexD(RSAkey* a, char* b)
	{	ScopyforSSL(b, BN_bn2hex(a->key->d));
		return b;
	}
	
	// put modulus into string
	char* hexN(RSAkey* a, char* b)
	{	ScopyforSSL(b, BN_bn2hex(a->key->n));
		return b;
	}
	
	// set public exponent to hex number contained in string 'a'
	void setE(RSAkey* a, char* b)
	{	if(a->key->e != 0)
		{	BN_clear_free(a->key->e);
		}
		BN_hex2bn(&a->key->e, b);
	}
	
	// set private exponent to hex number contained in string 'a'
	void setD(RSAkey* a, char* b)
	{	if(a->key->d != 0)
		{	BN_clear_free(a->key->d);
		}
		BN_hex2bn(&a->key->d, b);
	}
	
	// set modulus to hex number contained in string 'a'
	void setN(RSAkey* a, char* b)
	{	if(a->key->n != 0)
		{	BN_clear_free(a->key->n);
		}
		BN_hex2bn(&a->key->n, b);
	}
