...
-lsdl_net
	*/
	...
	// set the timeout for socket a to timeout number of microseconds
	/*void TCPsetTimeout(TCPsoc* a, int timeout)
	{	
		#if defined(_WIN32) || defined(WIN32) || defined(__CYGWIN__)
				// I dunno what to do...
		#else
			struct timeval theTimeout;
			theTimeout.tv_sec = timeout/1000000;
			theTimeout.tv_usec = timeout%1000000;
	...
	bool Accept(TCPsoc* listener, TCPsoc* a)
	{	
			#if defined(_WIN32) || defined(WIN32) || defined(__CYGWIN__)
				a->sock = SDLNet_TCP_Accept(listener->sock);		//try to accept a connection
				if(a->sock == NULL)
					return false;
				else
					return true;
				...
				if(-1==(a->sock = accept(listener->sock, (struct sockaddr *)&cliaddr, &len)))
	...
	// set the timeout for socket a to timeout number of microseconds
	/*void UDPsetTimeout(UDPsoc* a, int timeout)
	{	struct timeval theTimeout;
		theTimeout.tv_sec = timeout/1000000;
		theTimeout.tv_usec = timeout%1000000;
	...
	{	
		#if defined(_WIN32) || defined(WIN32) || defined(__CYGWIN__)
				a->packOut.address.host = addra->host;
				a->packOut.address.port = addra->port;
			#else
				a->dest.sin_port = addra->port;
				a->dest.sin_addr.s_addr = addra->host;
			#endif
	...
	// receives len bytes of data into container
	// blocks until len bytes is received
	// if into is NULL then you aren't given back an address
	// returns number of bytes received
	// returns 0 if a full packet has not been received at timeout
	...
	{	
				...
				int ready=0;
				while(ready==0)
				{	ready=SDLNet_UDP_Recv(a->sock, &packIn);
					SDL_Delay(10);
				}
				if(into!=0)
	...
	// attempts to send len bytes of data
	// returns number of bytes sent
	// if len isn't returned, there was an error
	inline int SendUDP(UDPsoc* a, void* data, int len)
	{	
		#if defined(_WIN32) || defined(WIN32) || defined(__CYGWIN__)
			...
			#else
				if(sendto(a->sock, data, len, 0, (struct sockaddr*)&(dest), sizeof(dest)) == -1)
					return true;
			#endif
