...
{	
	...
	{	int n, test;
		for(n=0; 1; n++)
		{	test = Recv(a, &(container[n]), 1);
			if(test==-1 || isIn_forKBHandling(container[n], ends))
				break;
		}
		container[n]=0;
		return n;
...
{	
	...
	{	
		#if defined(_WIN32) || defined(WIN32) || defined(__CYGWIN__)
			a->dest.host = addr->host;
			a->dest.port = addr->port;
	...
	{	
			...
			packet.len = len;
			packet.maxlen = len;
			packet.address.host = a->dest.host;
			packet.address.port = a->dest.port;
			
			if(SDLNet_UDP_Send(a->sock, -1, &packet) == 0)
			{	return true;
			}
		#else
