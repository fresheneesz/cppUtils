...
void gecharsNum(char* a, int num)
{	int n;
	for(n=0; n<num; n++)
	{	a[n]=getchar();
		if(a[n]==-1)
...
int gecharsEnds(char* a, char* ends)
{	int n;
...
// gets chacters until reaching one of a number of characters OR until grabbing enough charters
// returns the number of characters gotten
int gecharsNumEnds(char* a, char* ends, int maxlen)
{	int n;
	for(n=0; !isIn_forKBHandling(a[n]=getchar(), ends) && n<maxlen; n++)
	{	if(a[n]==-1)
		{	a[n]=0;
			return n-1;
		}
